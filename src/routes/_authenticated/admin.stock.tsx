...
function StockPage() {
  ...
  useEffect(() => {
    if (!isAdmin && allowedPos.length && !selectedPos) {
      setSelectedPos(allowedPos[0].id);
    }
  }, [isAdmin, allowedPos, selectedPos]);
  ...
  const posStocks = stocks.filter((s: any) => (s.pos_id ?? "") === (selectedPos || ""));
  const estimatedValue = posStocks.reduce((sum: { usd: number; fcfa: number }, row: any) => {
    ...
    <StaffShell title="Administration" requiredRole={["admin", "manager"]} requiredPermission="can_manage_stock">
      ...
      <div className="mt-6 flex flex-wrap items-end gap-4">
        <div>
          ...
          >
            {isAdmin && <option value="">— Stock central (web) —</option>}
        ...
        <table className="w-full text-sm">
          <thead className="bg-clay/50">
            <tr className="text-left text-xs uppercase tracking-widest">
              <th className="p-3">Produit</th><th className="p-3">Quantité</th><th className="p-3">Seuil alerte</th><th className="p-3">Valeur unitaire</th><th className="p-3">Statut</th><th></th>
            </tr>
          </thead>
          <tbody>
            {products.map((p: any) => {
...
function StockRow({ product, stock, low, onSave }: any) {
  ...
  return (
    <tr className="border-t border-border">
      ...
      <td className="p-3 text-xs text-muted-foreground">${product.price_usd} · {Number(product.price_fcfa).toLocaleString("fr-FR")} FCFA</td>
